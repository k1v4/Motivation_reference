...
type deletePhrase interface {
	DeletePhrase(id int64) error
}

func New(logger logger.Logger, deletePhrase deletePhrase, w http.ResponseWriter, r *http.Request) {
	...
	err = deletePhrase.DeletePhrase(int64(id))
	if err != nil {
		w.WriteHeader(http.StatusBadRequest)
